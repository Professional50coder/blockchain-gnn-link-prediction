...
def compute_link_probability(embedding_a, embedding_b):
    ...
    # Clip score to prevent overflow in exp
...
def color_fraud_score(val):
    """Color code fraud scores (lower is more suspicious)"""
    if val < -0.2:
        return 'background-color: #ffccbc'
    elif val < 0:
        return 'background-color: #fff9c4'
...
if "🏠 Overview" in section:
    ...
    if search_wallet:
        try:
            ...
            st.dataframe(filtered_edges.head(num_rows), width='stretch')
        ...
        st.dataframe(edges.head(num_rows), width='stretch')
    ...
    if 'wallet_address' in filtered_fraud.columns:
        ...
        st.dataframe(filtered_fraud, width='stretch', height=400)
        ...
        with st.spinner("Creating network visualization..."):
            ...
            if fig is not None:
                ...
                with st.expander("🔍 View Connected Wallets"):
                    st.dataframe(related_edges.head(50), width='stretch')
            else:
...
<div style='text-align: center; color: #666; padding: 2rem;'>
    ...
    <p style='font-size: 0.8rem; margin-top: 1rem;'>
        📊 Dashboard Version 1.1 | Last Updated: {}</p>
</div>
